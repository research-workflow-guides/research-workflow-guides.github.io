...
eyebrow: 선택
lead: 이전 원고와 현재 원고의 차이를 표시한 검토용 PDF가 필요할 때 사용하는 선택 흐름입니다.
toc:
  - id: 이-문서의-역할
    label: 이 문서의 역할
  - id: 이전-버전-준비
...
이 흐름은 지도교수, 공동저자, 심사자에게 수정 사항이 보이는 PDF를 전달해야 할 때 사용합니다.

흐름은 의도적으로 좁게 유지합니다. 두 TeX 파일을 준비하고, `diff_version.tex`를 생성하고, 빌드해서 결과 PDF를 확인합니다.
...
<div class="doc-step-pair">
  <div>
    <p>비교 기준 파일 이름이 분명해야 결과 PDF도 분명해집니다. 무엇을 기준으로 비교했는지 모호하면 검토용 PDF의 의미도 바로 흐려집니다.</p>
  </div>
...
현재 원고 상태는 `revised_version.tex`로 둡니다. 두 파일은 같은 문서의 이전 버전과 현재 버전이어야 하며, 서로 다른 초안이면 안 됩니다.
...
두 파일이 들어 있는 폴더에서 터미널을 연 뒤 아래 명령을 실행합니다.
...
<div class="doc-step-pair">
  <div>
    <p>두 TeX 파일이 있는 원고 폴더에서 터미널을 엽니다.</p>
  </div>
  <figure class="image-frame">
    ...
    <figcaption>비교 명령을 실행하기 전에 원고 프로젝트에서 터미널을 엽니다.</figcaption>
  </figure>
</div>
...
빌드하기 전에 `diff_version.tex`를 열고 정상적으로 생성됐는지 확인합니다. 간단한 확인만으로도 빌드 실패 전에 문제를 잡을 수 있습니다.

<div class="doc-step-pair">
  <div>
    <p>모든 줄을 읽을 필요는 없습니다. 파일이 존재하는지, 표시가 합리적으로 보이는지, 의도한 두 문서의 차이인지 확인하면 충분합니다.</p>
  </div>
  <figure class="image-frame">
    <img src="/assets/images/legacy/writing-optional/revision-pdf-latexdiff/2.3.2-5.png" alt="프로젝트 안에 생성된 diff_version 파일이 보이는 화면">
    <figcaption>생성된 diff 파일이 프로젝트 안에 나타나는지 먼저 확인합니다.</figcaption>
  </figure>
</div>

## diff 파일 빌드

생성된 diff 파일을 별도 대상 파일로 빌드합니다.

```bash
latexmk -pdf diff_version.tex
```

이 단계가 끝나면 삽입과 삭제가 표시된 검토용 PDF가 생성됩니다.
...
<div class="doc-step-pair">
  <div>
    <p>최종 점검은 시각적인 확인입니다. 표시가 읽기 좋은지, 수식이 온전한지, PDF가 의도한 비교 결과와 맞는지 확인합니다.</p>
  </div>
  <figure class="image-frame">
    <img src="/assets/images/legacy/writing-optional/revision-pdf-latexdiff/2.3.2-6.png" alt="수정된 원고 차이가 표시된 검토용 PDF">
    <figcaption>완성된 검토용 PDF는 삽입과 삭제를 읽기 쉽게 보여줘야 합니다.</figcaption>
  </figure>
</div>
...
- `latexmk -pdf diff_version.tex`가 읽을 수 있는 검토용 PDF를 만들었다
